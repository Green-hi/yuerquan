package com.greenhi.lalababy.adviewpager.adutils;
...
import com.greenhi.lalababy.adviewpager.adapter.ImageViewPagerAdapter;
...
public class AdViewpagerUtil {
     ...
     * @param urls
     */
    public AdViewpagerUtil(Context context, ViewPager viewPager, List<ImgBean> urls) {
     ...
     * @param urls
     */
    public AdViewpagerUtil(Context context, ViewPager viewPager, LinearLayout dotlayout, int dotsize, int dotoffset, List<ImgBean> urls) {
        this.context = context;
        this.viewPager = viewPager;
    ...
    private void initAdimgs(List<ImgBean> urls)
    {
    ...
    private void setImg(int length, List<ImgBean> urls)
    {
        if (urls.size() > 0) {
            imgsize = length;
            for (int i = 0; i < length; i++) {
                if (i < length - 2) {
                    final int index = i;
                    Glide.with(context) .load(urls.get(i).getPath()).into(mImageViews[i + 1]);
            ...
            Glide.with(context) .load(urls.get(urls.size() - 1).getPath()).into(mImageViews[0]);
            Glide.with(context) .load(urls.get(0).getPath()).into(mImageViews[length - 1]);
        }
    }
